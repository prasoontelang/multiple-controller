...
\n\
Multiple Controller:\n\
  net-split MAPPING          set the switch to network mapping\n\
\n\
Manager commands:\n\
...
static void cmd_split_network(struct vsctl_context *ctx)
{
    int length;
    char path[50] = "/tmp/mapping.txt";
    int fd = open(path,O_CREAT|O_WRONLY, 0777);
    lseek(fd, 0, SEEK_END);
    length = strlen(ctx->argv[1]);    
    write(fd, ctx->argv[1], length);
    write(fd, '\n', 1);
    perror("write");
    close(fd);
