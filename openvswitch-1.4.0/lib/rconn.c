...
static int permissionCheck( int, char [] );
static bool charCheck ( char );
static int getSwitchCount(char []);
static int allocSwitch(struct switchType **, int);
static int getControllerInput(struct switchType **, char [], int, int);
static void display(struct switchType [], int);
static void delete(struct switchType **, int);
static void setFilename(char []);
static long subnetmatch(long, struct network [], int);
static int lowermask(int);
static int uppermask(int);
static bool checkMatch(long, char []);
static int getCount(char [], int);
...
/* In scenarios of multiple controllers, source IP of the packet which
 * maps to the network_number, the corresponding controllerIP will be
 * used for forwarding the packets to the controller. */
struct network {
    char network_number [19];
    char controllerIP [16];
};

/* A wrapper around struct network, used to identify which set of 
 * network numbers to be searched in case of multiple controllers. */
struct switchType {
    char name[ 5 ];             /* Name of the switch */
    int assocCount;             /* Number of mappings it contain */
    struct network *assoc;
};
...
try_send(struct rconn *rc)
{
    ...
    FILE *f = fopen("/home/ashish/Documents/AOST/logs.txt","a+");
...
copy_to_monitor(struct rconn *rc, const struct ofpbuf *b)
{
    ...
    sw_name[j] = '\0';
    j =0;
    char filename[50] = "/tmp/mapping.txt";
    char c;
    int length = MAX_LEN, r;
    int iter, count;
    struct switchType * sw = NULL;
    int fd = open(filename, O_RDONLY);
    count = allocSwitch(&sw, fd);
    if (sw != NULL) {
        for(iter = 0; iter < count; iter++) {
            if(strcmp(sw_name, sw[iter].name) == 0) {
                uint64_t  ipaddr = b->l3 == NULL?0:1;
                if(ipaddr) {
                     uint64_t * ipaddrptr = (uint64_t *)b->l3 + 3;
                     controllerip = subnetmatch(*ipaddrptr, sw[iter].assoc,
                                                sw[iter].assocCount);
                }
                break;
            }
...
/* Returns the controller IP Address where the packet should be forwarded
 */
static long subnetmatch(long ipaddr, struct network net[], int size)
{
    int i;
    struct in_addr addr;
    for (i = 0; i < size; i++)
    {
        if (checkMatch(ipaddr, net[i].networkNumber))
        {
            inet_aton(net[i].controllerIP, &addr);
            return addr.s_addr;
        }
    }
}

/* Returns true if the ipaddr belongs to the network number else false.
 */
static bool checkMatch(long ipaddr, char network[])
{
    int i = 0, j = 0, k = 0, subnet;
    unsigned char *ip = &ipaddr;
    unsigned char netIP[4], netno[4];
    char sub_ip[4], sub[3]; 
    while (network[i] != '/')
    {
        if(network[i] != '.')
    ...
    i = i + 1;
    j = 0;
    while(i < strlen(network))
    {
        sub[j++] = network[i++];
    }
    subnet = atoi(sub);
    int mask = uppermask(subnet);
    int upmask, lomask;
    for (i = 0; i < 4; i++)
    {
        int check;
        check = uppermask(32);
        upmask = uppermask(8 * i);
        lomask = lowermask(8 * (3 - i));
        check = check ^ lomask;
        check = check ^ upmask;
        check = mask & check;
        check = (check >> (8 * (3 - i))) & (~uppermask(24));
        netIP[i] = ip[i] & check;
    }
    for (i = 0; i < 4; i++)
    {
...
/* Creates an absolute path to the filename from the root 
 */
static void setFilename(char filename[])
{
    ...
    {
        filename[j++] = hostname[i++];
    }
    i = 0;
    filename[j++] = '/';
    char map[] = "mapping.txt";
    while (i < strlen(map))
    {
        filename[j++] = map[i++];
    }
...
static int getSwitchCount(char fc[])
{
    int i, count = 0;
    int length = strlen(fc);
...
static void delete(struct switchType **sw, int count)
{
    struct switchType *this = *sw;
    free(*sw);
    *sw = NULL;
}

static int allocSwitch(struct switchType **head, int fd)
{
    struct switchType *sw;
    int i, j, count;
    char fileContent[MAX_LEN];
    int charRead = read(fd, fileContent, MAX_LEN);
    fileContent[charRead - 1] = '\0';
    int objno = 0;
    count = getSwitchCount(fileContent);
    (*head) = (struct switchType *)malloc(count * sizeof(struct switchType));
    sw = *head;
    for (i = 0; i < charRead; i++)
    {
        j = 0;
        while (i < charRead && fileContent[i] != ':')
        {
            if (j < 5)
            {
                sw[objno].name[j++] = fileContent[i++];
            } else {
                break;
            }
        }
        ...
        if (i < charRead)
        {
            i = getControllerInput(&sw, fileContent, objno, i);
        }
        objno = objno + 1;
    }
...
static int getControllerInput(struct switchType **head, char fileContent[], int objno, int i)
{
    int entries = getCount(fileContent, i);
    int j = 0, k;
    struct switchType *sw = *head;
    sw[objno].assoc = (struct network *)malloc(entries * sizeof(struct network));
    sw[objno].assocCount = entries;

    while(fileContent[i] != 's' && fileContent[i] != '\0')
    {
        k = 0;
        while(fileContent[i] != ',' && fileContent[i] && '\n' && fileContent[i] != '\0')
        {
            sw[objno].assoc[j].networkNumber[k++] = fileContent[i++];
        }
        sw[objno].assoc[j].networkNumber[k] = '\0';
        i = i + 1;
        k = 0;
        while (fileContent[i] != '\n' && fileContent[i] != ':' && fileContent[i] != '\0')
        {
            sw[objno].assoc[j].controllerIP[k++] = fileContent[i++];
        }
        i = i + 1;
        sw[objno].assoc[j].controllerIP[k] = '\0';
        j++;
    }
...
static int getCount(char fileContent[], int i)
{
    int count = 1;

    do
    {
        if (fileContent[i++] == ':' || fileContent[i] == '\0')
        {
            count = count + 1;
        }
    } while(fileContent[i] != '\0' && fileContent[i] != 's' && fileContent[i] != '\n');
