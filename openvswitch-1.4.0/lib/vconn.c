...
do_send(struct vconn *vconn, struct ofpbuf *msg)
{
    ...
    if (vconn->remote_ip==controllerip || controllerip==0) {	
        if (!VLOG_IS_DBG_ENABLED()) {
            COVERAGE_INC(vconn_sent);
            retval = (vconn->class->send)(vconn, msg);        
            uint64_t  ipaddr = msg->l3 == NULL?0:1;
    	    if(ipaddr) {
    		    uint64_t * ipaddrptr = (uint64_t *)msg->l3 + 3;
                uint64_t * ipaddrdest = (uint64_t *)msg->l3 + 4;
    	    }
        } else {
